...
@implementation VMVoteDetailCommentCell
...
//    }];
    ...
    [self.cardBackgroundView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.contentView).with.offset(-6 * HEIGHT_SCALE);
    ...
    [self.contentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //make.bottom.equalTo(self.cardBackgroundView).with.offset(-20 * HEIGHT_SCALE);
        //make.height.mas_equalTo(20 * HEIGHT_SCALE);
    }];
    
//    [self.cardBackgroundView addSubview:self.moreButton];
//    [self.moreButton mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(self.contentLabel.mas_bottom).with.offset(10 * HEIGHT_SCALE);
//        make.bottom.equalTo(self.cardBackgroundView).with.offset(-20 * HEIGHT_SCALE);
//        make.left.equalTo(self.cardBackgroundView).with.offset(20 * WIDTH_SCALE);
...
-(UIView *)remarkBackgroundView{
    if(!_remarkBackgroundView){
        _remarkBackgroundView = [[UIView alloc] init];
        _remarkBackgroundView.backgroundColor = [UIColor colorWithHexString:@"#F2F2F2"];
        _remarkBackgroundView.layer.cornerRadius = 8 * WIDTH_SCALE;
    }
    return _remarkBackgroundView;
}
...
-(UIButton *)moreButton{
    if(!_moreButton){
        ...
        _moreButton.titleLabel.font = [UIFont systemFontOfSize:8 * WIDTH_SCALE];
        [_moreButton setTitle:@"查看更多回复>>" forState:(UIControlStateNormal)];
        [_moreButton setTitleColor:[UIColor colorWithHexString:@"#A6B9FF"] forState:(UIControlStateNormal)];
